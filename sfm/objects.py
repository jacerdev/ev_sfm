...
def remove_observation(points_id_map, frame, point):
    """
    Removes the bidirectional link between a Frame and a Point3D.
    Uses Swap-and-Pop on the Frame side to preserve indices of other points.
    """
    # remove Frame reference from Point.observations
    for i, obs in enumerate(point.observations):
        if obs[0] == frame.id:
            point.observations.pop(i)
            break

    # find the index of the point in the Frame
    idx_to_remove = -1
    for idx, entry in enumerate(frame.points):
        if entry == point.id:
            idx_to_remove = idx
            break
    if idx_to_remove == -1: return  # link didn't exist in Frame, nothing to do

    #  Remove Point3D from Frame using Swap-and-Pop
    last_idx = len(frame.points) - 1

    # if already the last element, just pop, no indices shift
    if idx_to_remove == last_idx:
        frame.points.pop()
        frame.keypoints.pop()
        if frame.descriptors is not None: frame.descriptors.pop()
        if frame.scores is not None: frame.scores.pop()
        return

    # else: swap to preserve other indices
    moved_point_id = frame.points[-1]
    frame.points[idx_to_remove] = moved_point_id
    frame.keypoints[idx_to_remove] = frame.keypoints[-1]
    if frame.descriptors is not None: frame.descriptors[idx_to_remove] = frame.descriptors[-1]
    if frame.scores is not None: frame.scores[idx_to_remove] = frame.scores[-1]

    # Update the Point3D object that was moved.
    moved_point = points_id_map[moved_point_id]
    for i, obs in enumerate(moved_point.observations):
        if obs[0] == frame.id:
            moved_point.observations[i] = (frame.id, idx_to_remove)
            break

    # remove the duplicate data from the end
    frame.points.pop()
    frame.keypoints.pop()
    if frame.descriptors is not None: frame.descriptors.pop()
    if frame.scores is not None: frame.scores.pop()
